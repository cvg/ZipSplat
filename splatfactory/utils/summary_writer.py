...
from splatfactory import __module_name__, get_logger, settings
...
class SummaryWriter:
    ...
    def __init__(
        ...
        **wandb_kwargs,
        ...
        if self.use_wandb:
            ...
            name = name if name is not None else str(log_dir.relative_to(settings.TRAINING_PATH))
            if conf is not None:
                wandb_conf = OmegaConf.to_container(conf, resolve=True)
                wandb_conf = mappings.flatten_dict(wandb_conf)
            else:
                wandb_conf = None
    ...
    def add_plotly(self, tag: str, figure: Any, step: int | None = None):
        """Log a Plotly 3D figure to tensorboard or wandb, extracting mesh data."""
        if self.use_wandb:
            step = 1 if step == 0 else step
            wandb.log({tag: wandb.Plotly(figure)}, step=step)
