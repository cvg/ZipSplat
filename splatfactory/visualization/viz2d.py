...
2) Optionally: save a .png or .pdf plot (nice in papers!) with `save_plot`.
...
def plot_heatmaps(
    ...
    for i in range(len(axes)):
        a_ = alpha if isinstance(alpha, float) else alpha[i]

        if isinstance(heatmaps, torch.Tensor):
            heatmaps = heatmaps.detach().cpu().numpy()
        elif isinstance(heatmaps[i], torch.Tensor):
...
def fig_to_image(fig, close_fig=True):
